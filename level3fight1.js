...
class Level3Fight1 extends BaseScene {self
    constructor() {
        super('Level3Fight1');
    }
    ...
    create() {
        ...
        gameState.enemy1.health = 75;
        gameState.enemy1.healthMax = 75;
    ...
    // ---------------------------------- INTRO -------------------------------------    
        ...
        const levelimage = this.add.image(0,0, 'theDemonLair').setScale(0.87).setOrigin(0.02,0).setDepth(300);
        const leveltextTop = this.add.text(550, 270, 'Level 3:', levelTextConfig).setDepth(301).setOrigin(0.5);
        const leveltextBottom = this.add.text(550, 430, `The Demons' Lair`, levelTextConfig).setDepth(301).setOrigin(0.5); 
    ...
    // ---------------------------------- ENEMY'S TURN -------------------------------------      
        ...
        function updateEnemyActions() {
            ...
            if (!gameState.enemy2 || !gameState.enemy2.alive) {
                ...
                gameState.enemy1.actions = [ 
                    {key: `Intends to\nDeal 7 fire damage`, damage: 0, fire: 7, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: `Deals 7 Fire damage`, probability: 0.18 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.17 : 0) / 5},
                    {key: () => `Intends to\nDeal ${Math.round(15 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 15, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 15 damage', probability: 0.25 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.17 : 0) / 5},
                    {key: () => `Intends to\nDeal ${Math.round(18 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 18, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 18 damage', probability: 0.15 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.17 : 0) / 5},
                    ...
                    {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 20, poisonRemove: 0, strength: 0, armor: 0, text: 'Heals 20 HP', probability: (gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0 : 0.04},
                    {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 3, armor: 0, text: 'Gains 3 Strenght', probability: 0.08 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.17 : 0) / 5},
                    {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 4, text: 'Gains 4 Armor', probability: 0.12 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.17 : 0) / 5},
            ...
            if (gameState.enemy2) {
                gameState.enemy2.actions = [ 
                    ...
                    {key: `Intends to\nApply a debuff`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Applies a debuff', debuffCard: 'draw', probability: 0.40}
                ];
            }
        };
