...
class Level3Fight1 extends BaseScene {self
    ...
    create() {
        const self = this;      
        this.baseCreate('bakgrunnLair1'); //BaseScene method  
        ...
        const leveltextBottom = this.add.text(550, 430, 'The Lair', levelTextConfig).setDepth(301).setOrigin(0.5); 
    ...
    function updateEnemyActions() {
        ...
        gameState.enemy1.actions = [ 
            ...
            {key: () => `Intends to\nDeal ${Math.round(15 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 15, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 15 damage', probability: 0.24 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5},
            {key: () => `Intends to\nDeal ${Math.round(18 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 18, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 18 damage', probability: 0.17 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5},
        ...
        if (gameState.turn === 1) {
            gameState.enemy2.actions = [
                {key: `Intends to\nDeal 6 fire damage`, damage: 0, fire: 6, poison: 0, heal: 0, poisonRemove: 0, strength: 1, armor: 0, reduceTargetStrength: 0, reduceTargetArmor: 0, text: 'Deals 5 fire damage', probability: 1},
            ]
        } else {
    
            gameState.enemy2.actions = [ 
                {key: `Intends to\nDeal 7 fire damage`, damage: 0, fire: 7, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 7 fire damage', probability: 0.10 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 5},
    ...
    function selectEnemyAction(enemy) {
        ...
        function startNextLevel() {
            ...
            self.scene.start('Level3Fight2'); //sett lik neste level
            });
        }
