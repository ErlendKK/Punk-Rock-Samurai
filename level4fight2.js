...
class Level4Fight2 extends BaseScene {self
    ...
    create() {
        const self = this;      
        this.baseCreate('bakgrunnFortress2'); //BaseScene method  
    ...
    function performEnemyAction(enemy) {
        ...
        self.time.delayedCall(delaytime, () => {
            ...
            if (chosenAction.damage > 0 || chosenAction.fire > 0) {
                ...
                if (enemy === gameState.enemy1 || enemy === gameState.enemy2) { // NB! Level specific code for level 4! -> dont copy elsewhere!
                    const stealText = chosenAction.armor > 0 ? `Steals ${chosenAction.armor} Armor` : `Steals ${chosenAction.strength} Strength`
                    gameState.actionText = self.add.text(550, 300, `Deals ${enemy.damageTotal} damage\n${stealText}`, actionTextConfig).setOrigin(0.5);
    ...
    function updateEnemyActions() {
        ...
        if (gameState.turn === 1) {
            gameState.enemy1.actions = [
                {key: `Intends to\nSteal Armor`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 3, reduceTargetStrength: 0, reduceTargetArmor: 3,  text: 'Gains 3 armor', probability: 1},
            ] 
        } else if (gameState.turn === 2) {
            gameState.enemy1.actions = [
                {key: () => `Intends to\nDeal ${Math.round(12 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 12, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, reduceTargetStrength: 0, reduceTargetArmor: 3, text: 'Deals 12 damage', probability: 1},
            ]
    
        } else {

            gameState.enemy1.actions = [ 
                {key: `Intends to\nDeal 5 fire damage`, damage: 0, fire: 5, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 1, reduceTargetStrength: 0, reduceTargetArmor: 1, text: `Deals 5 fire damage`, probability: 0.15 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.20 : 0) / 5},
                ...
                {key: `Intends to\nSteal Armor`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 2, reduceTargetStrength: 0, reduceTargetArmor: 2, text: 'Steals 2 Armor', probability: 0.10 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.23 : 0) / 5},
        ...
        gameState.enemy2.actions = [ 
            {key: `Intends to\nDeal 5 fire damage`, damage: 0, fire: 5, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 1, reduceTargetStrength: 0, reduceTargetArmor: 1, text: `Deals 5 fire damage`, probability: 0.15 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5},
