...
class Level3Fight3 extends BaseScene {
    ...
    preload() {
        const assets = [
            { key: 'bakgrunnLair3', path: '../assets/images/backgrounds/bakgrunnLair3.jpg', loaded: false },
            { key: 'wraith1', path: '../assets/images/sprites/wraith1.png', loaded: false },
            { key: 'wraith2', path: '../assets/images/sprites/wraith2.png', loaded: false },
            { key: 'voidling', path: '../assets/images/sprites/voidling.png', loaded: false },
            { key: 'hellFire', path: '../assets/images/cards/hellFire.jpg', loaded: false }
        ];
    ...
    create() {
        ...
        const summonArgs = { x: 855, y: 555, health: 25, name: 'voidling', ID: 'enemy2' };
        const isSummoned1 = () => gameState?.enemy2?.alive;

        gameState.enemy1.actions = [
            // Predetermined turns
            new Action({ enemy: 1, turn: 1, debuffCard: 'doubleDiscard' }).setKey(`Intends to\nApply a debuff`),
            new Action({ enemy: 1, turn: 2, summonEnemy: summonArgs }).setKey("Intends to\nSummon a Voidling"),
            new Action({ enemy: 1, turn: 3, damage: 18 }).setKey(""),

            // Random turns
            new Action({ enemy: 1, summonEnemy: summonArgs, probability: () => isSummoned1() ? 0 : 1 }).setKey("Intends to\nSummon a Voidling"),
            new Action({ enemy: 1, damage: 15, probability: () => isSummoned1() ?  0.40 : 0 }).setKey(""),
            new Action({ enemy: 1, damage: 18, probability: () => isSummoned1() ?  0.20 : 0 }).setKey(""),
            new Action({ enemy: 1, heal: 10, strength: 1, probability: () => isSummoned1() ?  0.10 : 0 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, strength: 2, probability: () => isSummoned1() ?  0.10 : 0 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, debuffCard: 'discard', probability: () => isSummoned1() ?  0.15 : 0 }).setKey(`Intends to\nApply a debuff`),
            new Action({ enemy: 1, debuffCard: 'doubleDiscard', probability: () => isSummoned1() ?  0.05 : 0 }).setKey(`Intends to\nApply a debuff`),
        ];
        ...
        function handleCardEffects(card, target, isLastEnemy) {
            ...
            if (target.armor > 0 && cardEffects.damageTotal > 0) { //NB!
        ...
        */ 
        ...
        function summonNewEnemy(summonEnemy) {
            ...
            gameState[ID].actions = [
                ...
                new Action({ enemy: 2, fire: 4, pattern: 'even' }).setKey(`Intends to\nDeal 4 Fire Damage`),
            ];
