...
class Level3Fight2 extends BaseScene {
    ...
    preload() {
        const assets = [
            { key: 'bakgrunnLair2', path: '../assets/images/backgrounds/bakgrunnLair2.jpg', loaded: false },
            { key: 'skeleton1', path: '../assets/images/sprites/skeleton1.png', loaded: false },
            { key: 'skeleton2', path: '../assets/images/sprites/skeleton2.png', loaded: false },
    ...
    create() {
        ...
        const summonArgs1 = { x: 975, y: 557, health: 25, name: 'voidling', ID: 'enemy2' };
        const summonArgs2 = { x: 765, y: 557, health: 25, name: 'voidling', ID: 'enemy3' };
        const isSummoned1 = () => gameState?.enemy2?.alive;
        const isSummoned2 = () => gameState?.enemy3?.alive;
        const isAllSummoned = () => isSummoned1() && isSummoned2();
      
        gameState.enemy1.actions = [
            // Summon enemies
            new Action({ enemy: 1, summonEnemy: summonArgs1, probability: () => isSummoned1() ? 0 : 1 }).setKey("Intends to\nSummon a Voidling"),
            new Action({ enemy: 1, summonEnemy: summonArgs2, probability: () => (isSummoned2() || !isSummoned1()) ? 0 : 1 }).setKey("Intends to\nSummon a Voidling"),
           
            // Other actions
            new Action({ enemy: 1, fire: 5, probability: () => isAllSummoned() ? 0.20 : 0 }).setKey(`Intends to\nDeal 5 Fire Damage`),
            new Action({ enemy: 1, damage: 15, probability: () => isAllSummoned() ? 0.30 : 0 }).setKey(""),
            new Action({ enemy: 1, damage: 17, probability: () => isAllSummoned() ? 0.20 : 0 }).setKey(""),
            new Action({ enemy: 1, heal: 10, armor: 3, probability: () => isAllSummoned() ? 0.10 : 0 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, heal: 10, strength: 2, probability: () => isAllSummoned() ? 0.10 : 0 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, debuffCard: 'draw', probability: () => isAllSummoned() ? 0.10 : 0 }).setKey(`Intends to\nApply a debuff`),
        ];
