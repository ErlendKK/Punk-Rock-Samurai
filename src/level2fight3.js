...
class Level2Fight3 extends BaseScene {
    ...
    preload() {
        const assets = [
            { key: 'bakgrunnForest3', path: '../assets/images/backgrounds/bakgrunnForest3.jpg', loaded: false },
            { key: 'tree3', path: '../assets/images/sprites/tree3.png', loaded: false },
            { key: 'rooted', path: '../assets/images/cards/rooted.jpg', loaded: false },
            { key: 'goblin', path: '../assets/images/sprites/goblin1.png', loaded: false }
        ];
    ...
    create() {
        ...
        const summonArgs = { x: 840, y: 588, health: 30, name: 'goblin', ID: 'enemy2' };
        const isSummoned = () => gameState.enemy2?.alive;
        ...
        gameState.enemy1.actions = [
            ...
            new Action({ enemy: 1, turn: 3, summonEnemy: summonArgs }).setKey("Intends to\nSummon a Goblin"),
            new Action({ enemy: 1, turn: 4, heal: 10 }).setKey("Intends\nto Rest"),
            new Action({ enemy: 1, turn: 5, debuffCard: 'discard' }).setKey(`Intends to\nApply a debuff`),

            // Random turns
            new Action({ enemy: 1, summonEnemy: summonArgs, probability: () => isSummoned() ? 0 : 1 }).setKey("Intends to\nSummon a Goblin"),
            new Action({ enemy: 1, damage: 16, probability: () => isSummoned() ? 0.35 : 0 }).setKey(""),
            new Action({ enemy: 1, damage: 20, probability: () => isSummoned() ? 0.15 : 0 }).setKey(""),
            new Action({ enemy: 1, heal: 10, armor: 1, probability: () => isSummoned() ? 0.15 : 0 }).setKey("Intends\nto Rest"),
            new Action({ enemy: 1, debuffCard: 'discard', probability: () => isSummoned() ? 0.20 : 0 }).setKey(`Intends to\nApply a debuff`),
            new Action({ enemy: 1, strength: 2, probability: () => isSummoned() ? 0.15 : 0 }).setKey(`Intends to\nApply a buff`),
        ];
        ...
        async function startPlayersTurn(tweensDelay) {            
            ...
            console.log(`Turn: ${gameState.playersTurn}`);
        ...
        function handleAttackEffects(cardEffects, target) {
            ...
            // NB! Only for level 2!
            if (target === gameState.enemy1) { // NB! Only for level 2!
        ...
        */ 
        ...
        function summonNewEnemy(summonEnemy) {
            ...
            gameState[ID].actions = [
                // Random turns
                new Action({ enemy: gameState[ID], poison: 4, probability: 0.50 }).setKey(`Intends to\nPoison you`),
                new Action({ enemy: gameState[ID], damage: 10, probability: 0.30 }).setKey(""),
                new Action({ enemy: gameState[ID], heal: 10, armor: 5, probability: 0.20 }).setKey(`Intends to\nApply a buff`),
            ]
