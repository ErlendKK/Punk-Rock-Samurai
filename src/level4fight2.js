...
class Level4Fight2 extends BaseScene {
    ...
    preload() {
        const assets = [
            { key: 'bakgrunnFortress2', path: '../assets/images/backgrounds/bakgrunnFortress2.jpg', loaded: false },
            { key: 'monster3', path: '../assets/images/sprites/monster3.png', loaded: false },
            { key: 'monster4', path: '../assets/images/sprites/monster4.png', loaded: false },
            { key: 'sorcerer1', path: '../assets/images/sprites/sorcerer1.png', loaded: false },
            { key: 'monstrosity', path: '../assets/images/cards/monstrosity.jpg', loaded: false }
        ];
    ...
    create() {
        ...
        const isAlliesAlive = () => gameState.enemy1?.alive || gameState.enemy2?.alive;
      
        gameState.enemy1.actions = [
            // Predetermined pattern
            new Action({ enemy: 1, pattern: 'oneInFour', stealArmor: 2 }).setKey(`Intends to\nSteal Armor`),
            new Action({ enemy: 1, pattern: 'twoInFour', damage: 12 }).setKey(""),
            new Action({ enemy: 1, pattern: 'threeInFour', debuffCard: 'draw' }).setKey(`Intends to\nApply a debuff`),
            new Action({ enemy: 1, pattern: 'fourInFour', damage: 12 }).setKey(""),
        ];

        gameState.enemy2.actions = [
            // Predetermined pattern
            new Action({ enemy: 2, pattern: 'oneInFour', stealStrength: 1 }).setKey(`Intends to\nSteal Strength`),
            new Action({ enemy: 2, pattern: 'twoInFour', damage: 12 }).setKey(""),
            new Action({ enemy: 2, pattern: 'threeInFour', debuffCard: 'draw' }).setKey(`Intends to\nApply a debuff`),
            new Action({ enemy: 2, pattern: 'fourInFour', damage: 12 }).setKey(""),
        ];

        gameState.enemy3.actions = [
            // Allies alive - pattern
            new Action({ enemy: 3, pattern: () => isAlliesAlive() ? 'oneInThree' : null, debuffCard: 'draw' }).setKey(`Intends to\nApply a debuff`),
            new Action({ enemy: 3, pattern: () => isAlliesAlive() ? 'twoInThree' : null, heal: 10, teamEffect: true }).setKey('Intends to\nHeal his team'),
            new Action({ enemy: 3, pattern: () => isAlliesAlive() ? 'threeInThree' : null, strength: 1, armor: 2, teamEffect: true }).setKey(`Intends to\nBuff his team`),

            // Allies dead - pattern
            new Action({ enemy: 3, pattern: () => isAlliesAlive() ? null : 'even', damage: 12 }).setKey(""),
            new Action({ enemy: 3, pattern: () => isAlliesAlive() ? null : 'odd', damage: 15 }).setKey(""),
        ];
        ...
        async function handlePlayersTurnText() {
            const textCoordinates = { x: 825, y: 480, z: 21 };
        ...
        */ 
        ...
        async function performEnemyAction(enemy) {
            ...
            const { attackConditions, healConditions, buffConditions, stealCondition } = isEnemyEffectPlayed(chosenAction) 
        ...
        function isEnemyEffectPlayed(chosenAction) {
            ...
            return { attackConditions, healConditions, buffConditions, stealCondition };
        }
        ...
        function insertDebuffCard(enemy, chosenAction ) {
            return new Promise( async (resolve) => { 
                ...
                const addedDebuffCards = [
                    new Card({ key: cardKey, type: 'debuff', cost: cardData.cost, strength: -1, armor: -2, reUseable: true, drawCard: () => gameState.player.stancePoints > 0 ? 1 : 0}),
                    new Card({ key: cardKey, type: 'debuff', cost: cardData.cost, strength: -1, armor: -2, reUseable: true, drawCard: () => gameState.player.stancePoints > 0 ? 1 : 0}),
                ];
        ...
        function summonNewEnemy(summonEnemy) {
            ...
            gameState[ID].actions =  [
                // Patterned turns
                new Action({ enemy: 2, debuffCard: 'draw', pattern: 'odd' }).setKey(`Intends to\nApply a debuff`),
                new Action({ enemy: 2, fire: 5, pattern: 'even' }).setKey(`Intends to\nDeal 5 Fire Damage`),
            ];
