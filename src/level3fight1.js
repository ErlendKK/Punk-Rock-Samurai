...
class Level3Fight1 extends BaseScene {
    ...
    preload() {
        const assets = [
            { key: 'bakgrunnLair1', path: '../assets/images/backgrounds/bakgrunnLair1.jpg', loaded: false },
            { key: 'theDemonLair', path: '../assets/images/backgrounds/theDemonLair.jpg', loaded: false },
            { key: 'voidling', path: '../assets/images/sprites/voidling.png', loaded: false },
            { key: 'demon2', path: '../assets/images/sprites/demon2.png', loaded: false },
    ...
    create() {
        ...
        const summonArgs = { x: 945, y: 555, health: 25, name: 'voidling', ID: 'enemy2' };
        const isSummoned = () => gameState?.enemy2?.alive;
      
        gameState.enemy1.actions = [
            // Predetermined turns
            new Action({ enemy: 1, turn: 1, debuffCard: 'draw' }).setKey(`Intends to\nApply a debuff`),
            new Action({ enemy: 1, turn: 2, damage: 15 }).setKey(""),
            new Action({ enemy: 1, turn: 3, heal: 10, armor: 5, strength: 2 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, turn: 4, damage: 15 }).setKey(""),

            // Random turns
            new Action({ enemy: 1, summonEnemy: summonArgs, probability: () => isSummoned() ? 0 : 1 }).setKey("Intends to\nSummon a Voidling"),
            new Action({ enemy: 1, fire: 7, probability: () => isSummoned() ? 0.2 : 0 }).setKey(`Intends to\nDeal 7 Fire Damage`),
            new Action({ enemy: 1, damage: 15, probability: () => isSummoned() ? 0.3 : 0 }).setKey(""),
            new Action({ enemy: 1, damage: 18, probability: () => isSummoned() ? 0.1 : 0 }).setKey(""),
            new Action({ enemy: 1, heal: 10, armor: 3, probability: () => isSummoned() ? 0.1 : 0 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, heal: 10, strength: 2, probability: () => isSummoned() ? 0.15 : 0 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, debuffCard: 'draw', probability: () => isSummoned() ? 0.15 : 0 }).setKey(`Intends to\nApply a debuff`),
        ];
