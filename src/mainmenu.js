...
class Mainmenu extends Phaser.Scene {
    ...
    create() {
        ...
        const textConfig = { fontSize: '21px', fill: '#000000' }
        const headlineConfig = { fontSize: '38px', fill: '#000000' }
        ...
        menuButtons.push(
            newGameButton, loadGameButton, leaderBoardButton, creditsButton, instructionsButton
        )
        ...
        */
        ...
        function InitiatetNewGame() {
            ...
            menuButtons
                .filter(button => button !== newGameButton)
                .forEach(button => button.isPressed = false);

            // Create form for entering name.
            gameState.name = 'Enter your name...';
            const nameTextConfig = { fontSize: '35px', fill: '#000000' };
            ...
            sceneState.nameForm = self.add.graphics({x: 600, y: 435});
        ...
        function implementCursor() {
            const formCursorConfig = { fontSize: '48px', fill: '#000000' };
        ...
        this.input.keyboard.on('keydown', (event) => { 
            ...
            if (event.keyCode === 8 && gameState.name.length > 0) { // Implement backspace
        ...
        function selectDifficulty() {
            ...
            const textCoordinates = { x: x, y: 225, z: 201 };
        ...
        async function transitionToLevel(bonusCard, bonusCards) {
            gameConfig.cardsDealtSound.play({ volume: 1.2, seek: 0.12 });
            ...
            bonusCards
                .filter(card => card !== bonusCard)
                .forEach(card => card.disable().fadeOut(self));
        ...
        */
        ...
        function initiateLoadGame() {
            ...
            menuButtons
                .filter(button => button !== loadGameButton)
                .forEach(button => button.isPressed = false);
        ...
        function controlVersion(loadedState) {
            ...
            const confimaMessage = 
                `The game has been updated since this save was made (saved version: ${loadedState.version}, ` + 
                `current version: ${gameState.version}). Loading this file might cause issues. Do you still want to load?`;
        ...
        async function displayLeaderboard() {
            ...
            menuButtons
                .filter(button => button !== leaderBoardButton)
                .forEach(button => button.isPressed = false);

            const topScores = await self.getTopScores();
            const { x, xSpan, textOffsetX, textOffsetY, headlineOffsetY, radius } = textBox;
            const y = 400;
            const ySpan = 300;
        ...
        function displayTopFive(topScores, x, y) {
            topScores.forEach((score, idx) => {
                ...
                const scoreText = `${idx + 1}. Name: ${score.name}, Score: ${score.score}, Date: ${dateOnly}`;
        ...
        */
        ...
        function displayCredits() {
            ...
            menuButtons
                .filter(button => button !== creditsButton)
                .forEach(button => button.isPressed = false);
            ...
            const creditsText = self.add.text(x + textOffsetX, y + textOffsetY,
        ...
        function displayInstructions() {
            ...
            menuButtons
                .filter(button => button !== instructionsButton)
                .forEach(button => button.isPressed = false);
            ...
            const instructionsText = self.add.text(x + textOffsetX, y + textOffsetY, 

                `The game consists of four levels, each of which consists of three fights. At the start\n` +
                ...
                `Decks consists of ${gameConfig.minDeckSize} - ${gameConfig.maxDeckSize} cards, and can include ` +
                `at most ${gameConfig.maxCardExemplars} instances of the same card\n\n` +
