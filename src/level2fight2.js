...
class Level2Fight2 extends BaseScene {
    ...
    preload() {
        const assets = [
            { key: 'bakgrunnForest2', path: '../assets/images/backgrounds/bakgrunnForest2.jpg', loaded: false },
            { key: 'goblin1', path: '../assets/images/sprites/goblin1.png', loaded: false },
            { key: 'goblin2', path: '../assets/images/sprites/goblin2.png', loaded: false },
            { key: 'goblin3', path: '../assets/images/sprites/goblin3.png', loaded: false }
        ];
    ...
    create() {
        ...
        gameState.enemy1.actions = [
            // Predetermined turns
            new Action({ enemy: 1, turn: 1 }).setKey(`Intends to\nEvacuate Gold`),
            new Action({ enemy: 1, turn: 2, poison: 3 }).setKey("Intends to\nPoison you"),
            new Action({ enemy: 1, turn: 3, heal: 10, armor: 2 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 1, turn: 4, poison: 3 }).setKey("Intends to\nPoison you"),

            // Patterened turns
            new Action({ enemy: 1, damage: 10, pattern: 'oneInThree' }).setKey(""),
            new Action({ enemy: 1, poison: 3, pattern: 'twoInThree' }).setKey("Intends to\nPoison you"),
            new Action({ enemy: 1, heal: 10, armor: 2, pattern: 'threeInThree' }).setKey(`Intends to\nApply a buff`),
        ];

        gameState.enemy2.actions = [
            // Predetermined turns
            new Action({ enemy: 2, turn: 1 }).setKey(`Intends to\nEvacuate Gold`),
            new Action({ enemy: 2, turn: 2, poison: 3 }).setKey("Intends to\nPoison you"),
            new Action({ enemy: 2, turn: 3, heal: 10, armor: 2 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 2, turn: 4, poison: 3 }).setKey("Intends to\nPoison you"),

            // Patterened turns
            new Action({ enemy: 2, damage: 10, pattern: 'oneInThree' }).setKey(""),
            new Action({ enemy: 2, poison: 3, pattern: 'twoInThree' }).setKey("Intends to\nPoison you"),
            new Action({ enemy: 2, heal: 10, armor: 2, pattern: 'threeInThree' }).setKey(`Intends to\nApply a buff`),
        ];

        gameState.enemy3.actions = [
            // Predetermined turns
            new Action({ enemy: 3, turn: 1 }).setKey(`Intends to\nEvacuate Gold`),
            new Action({ enemy: 3, turn: 2, strength: 4 }).setKey(`Intends to\nApply a buff`),
            new Action({ enemy: 3, turn: 3, damage: 12 }).setKey(""),
            new Action({ enemy: 3, turn: 4, damage: 15 }).setKey(""),

            // Patterened turns
            new Action({ enemy: 3, poison: 4, pattern: 'oneInThree' }).setKey("Intends to\nPoison you"),
            new Action({ enemy: 3, damage: 10, pattern: 'twoInThree' }).setKey(""),
            new Action({ enemy: 3, heal: 10, armor: 2, pattern: 'threeInThree' }).setKey(`Intends to\nApply a buff`),
        ];
        ...
        function displayCountdownBox() {
            const countdownTextConfig = {fontSize: '105px', fontWeight: 'bold', fill: '#000000'};
            gameState.countdownTextBox = self.add.image(825, 225, 'goldCoin').setScale(0.18);
            gameState.countdownText = self.add.text(795, 180, `${countdownTimer}`, countdownTextConfig);
        }
        ...
        async function startPlayersTurn(tweensDelay) {            
            ...
            gameState.currentCards.forEach(card => {
                card.activate();
                console.log(`Turn: ${gameState.playersTurn}` + '. Card.key: ' + card.key + '. Activate: ' + card.isActive);
            })
        }
