class Level1fight1 extends BaseScene {
    ...
    create() {
        ...
        this.resetPlayer(gameState.player); //BaseScene method
        ...
        gameState.enemy1 = {
            name: 'Nazi Punk',
            sprite: this.add.sprite(710, 350, 'nazi').setScale(0.38).setFlipX(false).setInteractive(), //690 / 350 / .33
        ...
        gameState.enemy2 = {
            name: 'dummy',
            sprite: this.add.sprite(910, 340, 'nazi').setScale(0.01).setFlipX(true).setInteractive().setAlpha(0), //910 / 310 / .25
            healthBarColor: '0x00ff00',
            alive: true,
            actions: [],        
        ...
        function addStatsDisplay(character) {
            let statsTextConfig = { fontSize: '11px', fill: '#FFFFFF' }
            character.strengthAndArmorImage = self.add.sprite(character.sprite.x - 20, 460, 'strengthAndArmor').setScale(0.4).setInteractive().setDepth(20);
            character.armorText = self.add.text(character.sprite.x + 13, 462, `${character.armor}/${character.armorMax}`, statsTextConfig).setDepth(25);
            character.strengthText = self.add.text(character.sprite.x - 40, 462, `${character.strength}/${character.strengthMax}`, statsTextConfig).setDepth(25);
        }
        ...
        function addStanceBar() {
            ...
            gameState.stanceBar = self.add.graphics();
            gameState.stanceBar.fillStyle(0x00ff00, 1);
            
            let stanceTextConfig = { fontSize: '30px', fill: '#a9a9a9' }; // GREY FOR LEVEL 1-1     
        ...
        let level3image = this.add.image(0,0, 'theCity').setScale(0.87).setOrigin(0.02,0).setDepth(300);
        let level3textTop = this.add.text(550, 270, 'Level 1:', levelTextConfig).setDepth(301).setOrigin(0.5);
        let level3textBottom = this.add.text(550, 430, 'The City', levelTextConfig).setDepth(301).setOrigin(0.5); 
    ...
    // ---------------------------------- PLAYERS TURN -------------------------------------    
        ...
        function startPlayerTurn() {
            ...
            self.time.delayedCall(2200, () => { //timer: 2000
        ...
        function playCard(card, target) {
            ...
            gameState.enemies.forEach( enemy => {
                ...
                console.log(`turn: ${gameState.turn}: ${enemy.name} intention updated: ${enemy.chosenAction.key}`)
            })
    ...
    function selectEnemyAction(enemy) {
        ...
        function startNextLevel() {
            ...
            self.scene.start('Level1fight3'); //fiks til level1fight2
            });
        }
