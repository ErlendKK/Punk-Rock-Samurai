...
class Level2Fight2 extends BaseScene {
  constructor() {
    super('Level2Fight2');
  }
  preload() {
    const assets = [{
      key: 'bakgrunnForest2',
      path: '../assets/images/backgrounds/bakgrunnForest2.jpg',
      loaded: false
    }, {
      key: 'goblin1',
      path: '../assets/images/sprites/goblin1.png',
      loaded: false
    }, {
      key: 'goblin2',
      path: '../assets/images/sprites/goblin2.png',
      loaded: false
    }, {
      key: 'goblin3',
      path: '../assets/images/sprites/goblin3.png',
      loaded: false
    }];
  ...
  create() {
    ...
    this.baseCreate('bakgrunnForest2', sceneKey);
    this.resetPlayer(0.70, 540, 500);
    this.addGoldCounter();
    this.defineCardSlots();
    let countdownTimer = 5; //NB! Specific for level2fight2
    ...
    gameState.enemy1 = new Enemy('Junior Goblin\nApprentice of poison', 30, 0, 0, '');
    gameState.enemy1.sprite = this.add.image(1005, 570, 'goblin1').setScale(0.30).setFlipX(true);
    gameState.enemy2 = new Enemy('Junior Goblin\nArtisan of poison', 30, 0, 0, '');
    gameState.enemy2.sprite = this.add.image(1200, 570, 'goblin3').setScale(0.27).setFlipX(false);
    const enemy3Health = self.adjustForDifficulty(40, 40, 45);
    gameState.enemy3 = new Enemy('Senior Goblin\nMaster of poison', enemy3Health, 0, 0, '');
    gameState.enemy3.sprite = this.add.image(1395, 570, 'goblin2').setScale(0.375).setFlipX(false);
    gameState.enemy1.actions = [
    // Predetermined turns
    new Action({
      enemy: 1,
      turn: 1
    }).setKey("Intends to\nEvacuate Gold"), new Action({
      enemy: 1,
      turn: 2,
      poison: 3
    }).setKey("Intends to\nPoison you"), new Action({
      enemy: 1,
      turn: 3,
      heal: 10,
      armor: 2
    }).setKey("Intends to\nApply a buff"), new Action({
      enemy: 1,
      turn: 4,
      poison: 3
    }).setKey("Intends to\nPoison you"),
    ...
    gameState.characters.forEach(character => {
      character.setScene(this).setSpriteAttributes().addHealthBar().addManaBar().addStatsDisplay(685);
    ...
    async function initiatePlayerTurn() {
      ...
      if (gameState.turn > 2) updateCountdownBox(); // NB! Level-specific function
    ...
    function displayCountdownBox() {
      const countdownTextConfig = {
        fontSize: '105px',
        fontWeight: 'bold',
        fill: '#000000'
      };
      gameState.countdownTextBox = self.add.image(825, 225, 'goldCoin').setScale(0.18);
      gameState.countdownText = self.add.text(795, 180, "".concat(countdownTimer), countdownTextConfig);
    }
    function updateCountdownBox() {
      if (countdownTimer > 0) countdownTimer -= 1;
      if (countdownTimer && gameState.turn > 2) gameState.countdownText.setText("".concat(countdownTimer));
      if (countdownTimer === 0) {
    ...
    async function startPlayersTurn(tweensDelay) {
      ...
      gameState.currentCards.forEach(card => {
        card.activate();
        console.log("Turn: ".concat(gameState.playersTurn) + '. Card.key: ' + card.key + '. Activate: ' + card.isActive);
      });
    }
    ...
    */
    ...
    async function initiateEnemyTurnOneEvent() {
      ...
      const secondDelayTime = 3000;
      const thirdDelayTime = 3000;
      const textConfig = {
        fontSize: '45px',
        fill: '#ff0000',
        fontWeight: 'bold'
      ...
      const textContent = 'The Goblins are\nevacuating their gold!';
      const enemTurnOneText = new TextBox(self, textContent, textCoordinates, textConfig, alpha);
      await self.delay(secondDelayTime);
      enemTurnOneText.destroy();
      const newTextContent = "Defeat them in\n5 turns to steal\nwhat's left!";
      const enemTurnOneTextTwo = new TextBox(self, newTextContent, textCoordinates, textConfig, alpha);
      await self.delay(thirdDelayTime);
      enemTurnOneTextTwo.destroy();
    }
