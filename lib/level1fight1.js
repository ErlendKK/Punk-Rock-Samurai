...
class Level1Fight1 extends BaseScene {
  constructor() {
    super('Level1Fight1');
  }
  preload() {
    const assets = [{
      key: 'theCity',
      path: '../assets/images/backgrounds/theCity.jpg',
      loaded: false
    }, {
      key: 'bakgrunnCity1',
      path: '../assets/images/backgrounds/bakgrunnCity1.jpg',
      loaded: false
    }, {
      key: 'nazi',
      path: '../assets/images/sprites/nazi.png',
      loaded: false
    }];
  ...
  create() {
    ...
    gameState.redrawButton = new Button(self, 'Redraw', activateRedrawButton, 1350, 75);
    ...
    gameState.characters.forEach(character => {
      character.setScene(this).setSpriteAttributes().addHealthBar().addManaBar().addStatsDisplay(705);
    ...
    addPermanent(gameState.deck.find(c => c.freePermanent === true));
    ...
    */
    ...
    function handleLustForLife(card) {
      ...
      // Activate if not free permanent (level1-fight1).
      if (gameState.lustForLifeCounter === 1 && !card.freePermanent) {
      ...
      for (let i = 0; i < numCards; i++) {
        self.time.delayedCall(i * 75, () => {
          if (gameState.currentCards.length >= numCardsLimit) {
            self.cameras.main.shake(70, .002, false);
            console.log('gameState.currentCards.length: ' + gameState.currentCards.length + 'numCardsLimit: ' + numCardsLimit); // remove this
            return;
          }
