...
class Character {
  ...
  addHealthBar() {
    ...
    this.healthBarText = this.scene.add.text(x + textDisplacementX, y + 8, "".concat(this.health, "/").concat(this.healthMax), textConfig);
  ...
  addManaBar() {
    ...
    const textContent = "".concat(this.mana, "/").concat(this.manaMax);
  ...
  addStatsDisplay() {
    let y = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 420;
    const textConfig = {
      fontSize: '16px',
      fill: '#FFFFFF'
    };
    ...
    this.armorText = this.scene.add.text(x + 30, y + 5, "".concat(this.armor, "/").concat(this.armorMax), textConfig).setDepth(25);
    this.strengthText = this.scene.add.text(x - 60, y + 5, "".concat(this.strength, "/").concat(this.strengthMax), textConfig).setDepth(25);
    return this;
  }
  ...
  updateHealthBar() {
    ...
    this.healthBarText.setText("".concat(this.health, "/").concat(this.healthMax));
  ...
  heal(hp) {
    let poisonReduction = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  ...
  remove() {
    ...
    const characterObjects = [this.healthBarBackground, this.healthBarFrame, this.healthBar, this.healthBarText, this.manaBarBackground, this.manaBarFrame, this.manaBar, this.manaBarText, this.strengthAndArmorImage, this.strengthText, this.armorText, this.descriptionBackground, this.descriptionText, this.intentionText, this.intentionBackground];
...
class Enemy extends Character {
  constructor(name, healthMax, strengthBase, armorBase) {
    let debuffKey = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
  ...
  updateStats() {
    ...
    this.armor = Math.min(this.armorMax, this.armorBase + this.armorTurn);
    this.armorText.setText("".concat(this.armor, "/").concat(this.armorMax));
    this.strengthText.setText("".concat(this.strength, "/").concat(this.strengthMax));
    return this;
  }
  ...
  normalizeActions() {
    ...
    const assignDynamicProbablity = action => {
      const probabilityLookUpTable = {
        even: () => {
          action.probability = gameState.turn % 2 === 0 ? 1 : 0;
        },
        odd: () => {
          action.probability = gameState.turn % 2 !== 0 ? 1 : 0;
        },
        oneInThree: () => {
          action.probability = gameState.turn % 3 === 0 ? 1 : 0;
        },
        twoInThree: () => {
          action.probability = gameState.turn % 3 === 1 ? 1 : 0;
        },
        threeInThree: () => {
          action.probability = gameState.turn % 3 === 2 ? 1 : 0;
        },
        oneInFour: () => {
          action.probability = gameState.turn % 4 === 1 ? 1 : 0;
        },
        twoInFour: () => {
          action.probability = gameState.turn % 4 === 2 ? 1 : 0;
        },
        threeInFour: () => {
          action.probability = gameState.turn % 4 === 3 ? 1 : 0;
        },
        fourInFour: () => {
          action.probability = gameState.turn % 4 === 0 ? 1 : 0;
        }
      };
    ...
    console.log("enemy: ".concat(this.name, " . TotalProbability: ").concat(totalProbability));
  ...
  displayIntention() {
    ...
    this.intentionText = this.scene.add.text(this.x + 15, this.y - this.height / 2 - 63, "".concat(chosenAction.key), textConfig);
...
class Player extends Character {
  ...
  updateManaBar() {
    ...
    this.manaBarText.setText("".concat(this.mana, "/").concat(this.manaMax));
    return this;
  }
  ...
  updateStance() {
    ...
    switch (points) {
      ...
      case 3:
        ...
        break;
      default:
        console.error("Unexpected value for points: ".concat(points));
        return this;
    }
    ;
  ...
  updateStanceBar(points) {
    ...
    gameState.stanceBar.fillRect(gameState.stanceBarStartX + 300 + startXOffset, gameState.stanceBarMargin, Math.abs(points) * 100, gameState.stanceBarHeight);

    // Update stance text with updated stance
    gameState.stanceText.setText("Stance: ".concat(this.stance));
  }
  ...
  updateState() {
    return this.resetStanceAndMana() // NB! Call this first!
    .resetStrengthAndArmor().updateManaBar().updateHealthBar().updateStats();
  }
