...
class Level3Fight3 extends BaseScene {
  constructor() {
    super('Level3Fight3');
  }
  preload() {
    const assets = [{
      key: 'bakgrunnLair3',
      path: '../assets/images/backgrounds/bakgrunnLair3.jpg',
      loaded: false
    }, {
      key: 'wraith1',
      path: '../assets/images/sprites/wraith1.png',
      loaded: false
    }, {
      key: 'wraith2',
      path: '../assets/images/sprites/wraith2.png',
      loaded: false
    }, {
      key: 'voidling',
      path: '../assets/images/sprites/voidling.png',
      loaded: false
    }, {
      key: 'hellFire',
      path: '../assets/images/cards/hellFire.jpg',
      loaded: false
    }];
  ...
  create() {
    ...
    this.baseCreate('bakgrunnLair3', sceneKey);
    this.resetPlayer(0.615, 525, 525);
    this.addGoldCounter();
    this.defineCardSlots();
    const kaidanshuArmor = self.adjustForDifficulty(12, 13, 13); // NB Specific for level 3-3
    ...
    const enemy1Health = self.adjustForDifficulty(45, 50, 60);
    gameState.enemy1 = new Enemy('Kaidanshu', enemy1Health, 0, 12, 'hellFire');
    gameState.enemy1.sprite = this.add.image(1155, 458, 'wraith2').setScale(0.54).setFlipX(false);
    gameState.enemy1.intentionDepth = 25;
    const summonArgs = {
      x: 855,
      y: 555,
      health: 25,
    ...
    const isSummoned1 = () => {
      var _gameState;
      return (_gameState = gameState) === null || _gameState === void 0 || (_gameState = _gameState.enemy2) === null || _gameState === void 0 ? void 0 : _gameState.alive;
    };
    ...
    new Action({
      enemy: 1,
      turn: 1,
      debuffCard: 'doubleDiscard'
    }).setKey("Intends to\nApply a debuff"), new Action({
      enemy: 1,
      turn: 2,
      summonEnemy: summonArgs
    }).setKey("Intends to\nSummon a Voidling"), new Action({
      enemy: 1,
      turn: 3,
      damage: 18
    }).setKey(""),
    ...
    new Action({
      enemy: 1,
      summonEnemy: summonArgs,
      probability: () => isSummoned1() ? 0 : 1
    }).setKey("Intends to\nSummon a Voidling"), new Action({
      enemy: 1,
      damage: 15,
      probability: () => isSummoned1() ? 0.40 : 0
    }).setKey(""), new Action({
      enemy: 1,
      damage: 18,
      probability: () => isSummoned1() ? 0.20 : 0
    }).setKey(""), new Action({
      enemy: 1,
      heal: 10,
      strength: 1,
      probability: () => isSummoned1() ? 0.10 : 0
      ...
      enemy: 1,
      debuffCard: 'discard',
      probability: () => isSummoned1() ? 0.15 : 0
    }).setKey("Intends to\nApply a debuff"), new Action({
      enemy: 1,
      debuffCard: 'doubleDiscard',
      probability: () => isSummoned1() ? 0.05 : 0
    }).setKey("Intends to\nApply a debuff")];
    gameState.enemies = [gameState.enemy1];
    ...
    this.addStanceBar(gameState.player, '#808080'); // colors: light:#a9a9a9 - medium:#808080 - dark:#696969 - vdark:#303030
    ...
    async function initiatePlayersTurnOneEvent() {
      ...
      const textContent = "Kaidanshu start each\nturn with ".concat(kaidanshuArmor, " Armor,\nand lose 1 Armor\nper damage recieved");
    ...
    function initiateCardActivation(card) {
      console.log('initiateCardActivation called'); // Remove this
    ...
    function handleCardEffects(card, target, isLastEnemy) {
      ...
      //NB! LEVEL-SPECIFIC CONDITION FOR LEVEL 3-3
      if (target.armor > 0 && cardEffects.damageTotal > 0) {
        //NB!
        target.armor = Math.max(0, target.armor - cardEffects.damageTotal); //NB!
      } //NB!
    ...
    */
    ...
    async function startEnemyTurn(enemy) {
      ...
      if (gameState.playersTurn) {
        ...
        // NB Specific for level 3-3
        if (enemy.name === 'Kaidanshu') {
          // NB!
          enemy.armor = kaidanshuArmor; // NB!
        }
        // NB Specific for level 3-3
    ...
    function fadeInEnemy(enemy, duration) {
      ...
      self.tweens.add({
        ...
        onComplete: function onComplete() {
          ...
          enemy.setScene(self).setSpriteAttributes().addHealthBar().addStatsDisplay(680);
