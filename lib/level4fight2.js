...
class Level4Fight2 extends BaseScene {
  constructor() {
    super('Level4Fight2');
  }
  preload() {
    const assets = [{
      key: 'bakgrunnFortress2',
      path: '../assets/images/backgrounds/bakgrunnFortress2.jpg',
      loaded: false
    }, {
      key: 'monster3',
      path: '../assets/images/sprites/monster3.png',
      loaded: false
    }, {
      key: 'monster4',
      path: '../assets/images/sprites/monster4.png',
      loaded: false
    }, {
      key: 'sorcerer1',
      path: '../assets/images/sprites/sorcerer1.png',
      loaded: false
    }, {
      key: 'monstrosity',
      path: '../assets/images/cards/monstrosity.jpg',
      loaded: false
    }];
  ...
  create() {
    ...
    this.baseCreate('bakgrunnFortress2', sceneKey);
    this.resetPlayer(0.53, 525, 560);
    this.addGoldCounter();
    ...
    if (gameState.lustForLifeCounter >= 5) {
      ...
      gameState.lustForLifeButton.setPointerText(" Heal 7 HP\nCost: ".concat(gameState.lustForLifeCost, " gold"));
      gameState.buttons.push(gameState.lustForLifeButton);
    }
    gameState.buttons.forEach(button => button.disable());

    // Initialize characters
    const enemy1Health = self.adjustForDifficulty(35, 40, 45);
    gameState.enemy1 = new Enemy('Gloomhoof\nSteals Armor', enemy1Health, 0, 0, 'monstrosity');
    gameState.enemy1.sprite = this.add.image(870, 530, 'monster3').setScale(0.55).setFlipX(false);
    const enemy2Health = self.adjustForDifficulty(45, 50, 55);
    gameState.enemy2 = new Enemy('Darktuft\nSteals Strength', enemy2Health, 0, 0, 'monstrosity');
    gameState.enemy2.sprite = this.add.image(1200, 525, 'monster4').setScale(0.60).setFlipX(true);
    const enemy3Health = self.adjustForDifficulty(50, 55, 60);
    gameState.enemy3 = new Enemy('Ampward', enemy3Health, 0, 0, 'monstrosity');
    gameState.enemy3.sprite = this.add.image(1485, 550, 'sorcerer1').setScale(0.55).setFlipX(false);
    const isAlliesAlive = () => {
      var _gameState$enemy, _gameState$enemy2;
      return ((_gameState$enemy = gameState.enemy1) === null || _gameState$enemy === void 0 ? void 0 : _gameState$enemy.alive) || ((_gameState$enemy2 = gameState.enemy2) === null || _gameState$enemy2 === void 0 ? void 0 : _gameState$enemy2.alive);
    };
    ...
    // Allies alive - pattern
    new Action({
      enemy: 3,
      pattern: () => isAlliesAlive() ? 'oneInThree' : null,
      debuffCard: 'draw'
    }).setKey("Intends to\nApply a debuff"), new Action({
      enemy: 3,
      pattern: () => isAlliesAlive() ? 'twoInThree' : null,
      heal: 10,
      teamEffect: true
    }).setKey('Intends to\nHeal his team'), new Action({
      enemy: 3,
      pattern: () => isAlliesAlive() ? 'threeInThree' : null,
      strength: 1,
      armor: 2,
      teamEffect: true
    }).setKey("Intends to\nBuff his team"),
    ...
    new Action({
      enemy: 3,
      pattern: () => isAlliesAlive() ? null : 'even',
      damage: 12
    }).setKey(""), new Action({
      enemy: 3,
      pattern: () => isAlliesAlive() ? null : 'odd',
      damage: 15
    }).setKey("")];
    ...
    gameState.characters.forEach(character => {
      character.setScene(this).setSpriteAttributes().addHealthBar().addManaBar().addStatsDisplay(680);
    ...
    function welcomePlayerToLevel() {
      const levelimage = self.add.image(0, 0, 'theDemonLair');
      ...
      const leveltextTop = self.add.text(825, 405, 'Level 3:', levelTextConfig);
      const leveltextBottom = self.add.text(825, 645, 'Jigoku Lair', levelTextConfig);
    ...
    async function handleStartFightText() {
      ...
      const {
        level,
        fight
      } = self.extractLevelFightFromName(self.scene.key);
      const startTextConfig = {
        fontSize: '100px',
        fill: '#ff0000',
        fontFamily: 'Rock Kapak'
      };
      const startTextContent = "Level ".concat(level, "\nFight ").concat(fight, "!");
    ...
    function activateChemicalWarfare() {
      ...
      const textContent = "Enemies get +".concat(gameState.chemicalWarfare, " Poison");
      const textConfig = {
        fontSize: '45px',
        fill: '#ff0000'
      };
    ...
    async function drawCards(numCards) {
      ...
      for (let i = 0; i < numCards; i++) {
        ...
        const {
          card,
          cardDepth
        } = drawCardAndCreateSprite(i, startSlotIndex);
        card.slot = slot;
    ...
    async function startPlayersTurn(tweensDelay) {
      ...
      console.log("gameState.playersTurn: ".concat(gameState.playersTurn));
    ...
    function drawCardAndCreateSprite(i) {
      let startSlotIndex = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    ...
    function initiateCardActivation(card) {
      ...
      console.log("activated card: ".concat(card.key, "\nCost: ").concat(card.costPlayed));
    ...
    async function playCard(card, target) {
      let isLastEnemy = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    ...
    function handleCardEffects(card, target, isLastEnemy) {
      ...
      const {
        isBuffPlayed,
        isAttackPlayed
      } = isBuffOrAttackPlayed(cardEffects, target);
    ...
    function handleAttackEffects(cardEffects, target) {
      ...
      gameConfig.attackSound.play({
        volume: 0.8
      });
      gameState.player.attackTweens(60);
      gameState.score.damageDealt += cardEffects.damageTotal;
      console.log("".concat(cardEffects.damagePlayed, " Physical Damage and ").concat(cardEffects.firePlayed, " Fire Damage was dealt to ").concat(target.name));
    ...
    function activateSpecialCards(target, card) {
      ...
      const cardActions = {
        ...
        'coverCharge': () => {
          gameState.player.stancePoints > 1 ? earnGold(1) : null;
        },
      ...
      // Handle 'usesTillDepletion' if applicable
      if (card.usesTillDepletion >= 0) {
        const textContent = card.usesTillDepletion > 0 ? "Remaining uses: ".concat(card.usesTillDepletion) : "Card depleted!";
    ...
    function activateRiotRonin(card, target) {
      card.goldCost++;
      target.chosenAction = new Action({
        enemy: target,
        skip: true
      }).setKey("Intends to\nSkip a Turn");
    }
    ...
    function addActionText(card, cardEffects) {
      ...
      if (damageTotal) actionTextContent.push("Deals ".concat(damageTotal, " damage"));
      if (poisonPlayed) actionTextContent.push("Deals ".concat(poisonPlayed, " Poison"));
      if (strengthPlayed) actionTextContent.push("Gains ".concat(strengthPlayed, " Strength"));
      if (armorPlayed) actionTextContent.push("Gains ".concat(armorPlayed, " Armor"));
      if (healPlayed) actionTextContent.push("Heals ".concat(healPlayed, " HP"));
      if (poisonRemovePlayed) actionTextContent.push("Heals ".concat(poisonRemovePlayed, " Poison"));
    ...
    function normalizeCard(card, target) {
      let isLastEnemy = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    ...
    function handleRebelHearth() {
      ...
      gameConfig.healSound.play({
        volume: 0.5
      });
      self.animatePermanent('rebelHeart');
      const textContent = "Rebel Hearth healed ".concat(realizedHealthGain, " HP");
      addInfoTextBox(textContent);
    }
    ...
    */
    ...
    function handleLifeSteal() {
      ...
      const textContent = "You stole ".concat(stolenHealthRealized(), " HP");
      const textConfig = {
        fontSize: '45px',
        fill: '#ff0000'
      };
    ...
    async function performEnemyAction(enemy) {
      ...
      //NB Specific for level 4!
      if (chosenAction.teamEffect) {
        gameState.enemies.forEach(e => {
          e.heal(chosenAction.heal);
          e.gainStrengthAndArmor(chosenAction.strength, chosenAction.armor);
          e.updateStats();
        });
      } else {
      ...
      const {
        ...
        buffConditions,
        stealCondition
      } = isEnemyEffectPlayed(chosenAction);
      if (attackConditions) {
        ...
        self.updateTextAndBackground(gameState.actionText, gameState.actionTextBackground, chosenAction.text);
        enemy.powerUpTweens();
      } else if (stealCondition) {
        handleEnemySteal(enemy, chosenAction);
    ...
    function isEnemyEffectPlayed(chosenAction) {
      ...
      const stealCondition = chosenAction.stealArmor || chosenAction.stealStrength;
      return {
        attackConditions,
        healConditions,
        buffConditions,
    ...
    function handleEnemySteal(enemy, chosenAction) {
      gameConfig.powerUpSound.play({
        volume: 0.2
      });
      self.updateTextAndBackground(gameState.actionText, gameState.actionTextBackground, chosenAction.text);
      enemy.powerUpTweens();
      if (chosenAction.stealStrength) {
        gameState.player.strengthBase -= chosenAction.stealStrength;
        enemy.strengthBase += chosenAction.stealStrength;
      } else if (chosenAction.stealArmor) {
        gameState.player.armorBase -= chosenAction.stealArmor;
        enemy.armorBase += chosenAction.stealArmor;
      }
    }
    ...
    function handleEnemyAttack(enemy, chosenAction) {
      ...
      console.log("enemy.damageTotal: ".concat(enemy.damageTotal));
      gameState.player.health -= enemy.damageTotal;
      gameState.score.damageTaken += enemy.damageTotal;
      let actionTextContent = chosenAction.poison > 0 ? chosenAction.text : "Deals ".concat(enemy.damageTotal, " damage");
    ...
    async function concludeEnemyAction(enemy) {
      ...
      // NB! Adjusted for level 4
      let delaytime = enemy.chosenAction.debuffCard === "doubleDiscard" ? 4200 : gameState.debuffCardPlayed ? 1800 : 900;
      if (enemy.chosenAction.teamEffect) delaytime += 500; // For level4
      await self.delay(delaytime);
      // NB! Adjusted for level 4
    ...
    function insertDebuffCard(enemy, chosenAction) {
      return new Promise(async resolve => {
        ...
        console.log('insertDebuffCard called'); // Remove this
        ...
        const addedDebuffCards = [new Card({
          key: cardKey,
          type: 'debuff',
          cost: cardData.cost,
          strength: -1,
        ...
        // Inform the player
        let actionTextContent = "Adds 2 ".concat(cardData.name, " to your ").concat(pileName);
    ...
    function summonNewEnemy(summonEnemy) {
      ...
      gameState[ID] = new Enemy(NamePropper, health, 0, 0, 'hellFire');
      ...
      let actionTextContent = "Summons a ".concat(name);
      self.updateTextAndBackground(gameState.actionText, gameState.actionTextBackground, actionTextContent);
    ...
    // Handle Victory
    async function initiateVictory() {
      // NB! Modified for level 2-2
      gameState.score.numberOfTurns += gameState.turn;
      ...
      const levelCompleteText = fight === 3 ? "You have completed Level ".concat(level, "\nHealth is resorted to ").concat(gameState.player.healthMax, "/").concat(gameState.player.healthMax) : "";
    ...
    function createEndOfFightMenu() {
      ...
      const collectGoldText = "Collect ".concat(victoryElements.goldAmount, " Gold");
      ...
      victoryElements.gameOverObjects = [...victoryElements.rewardButtons, victoryElements.gameOverText];
    }
    ...
    function pickCardAndAddToDeck() {
      var _gameState$latestDraw;
      let depth = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 102;
      const x = 510;
      const y = 510;
      const spacing = 315;
      const redrawCost = 1;
      ...
      (_gameState$latestDraw = gameState.latestDraw) === null || _gameState$latestDraw === void 0 || _gameState$latestDraw.forEach(card => {
    ...
    function pickCardAndRemoveFromDeck() {
      let depth = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 220;
      const spacing = 180;
    ...
    function handleRemoveCardPointerup(removedCard) {
      ...
      console.log("Deck length before removal: ".concat(gameState.deck.length));
    ...
    function initiatAddCardRedrawButton(y, redrawCost, bonusCards) {
      ...
      const handlerArgs = {
        y: y,
        redrawCost: redrawCost,
        bonusCards: bonusCards,
        depth: depth
      };
      const redrawText = "   Redraw\nCost: ".concat(redrawCost, " Gold");
    ...
    function createShop(shopElements) {
      ...
      shopElements.textConfig = {
        fontSize: '75px',
        fill: '#ff0000'
      };
      const healDescription = " Gain ".concat(shopElements.healAmount, " HP\nCost: ").concat(shopElements.healCost, " Gold");
      const addCardDescription = " Buy 1 Card\nCost: ".concat(shopElements.addCardCost, " Gold");
      const removeCardDescription = "Deplete 1 card\n Cost: ".concat(shopElements.removeCardCost, " Gold");
      const x = 255;
      const y = 410;
      const offset = 130;
      let i = 0;
      ...
      shopElements.shopButtons = [shopElements.shopHealButton, shopElements.shopExitButton, shopElements.shopAddCardButton, shopElements.shopRemoveCardButton];
    ...
    function handleShopHealPointerup(sE) {
      ...
      const {
        isHealEnabled
      } = IsShopButtonEnabled(sE.healCost, sE.levelComplete);
      if (!isHealEnabled) {
        self.cameras.main.shake(50, .0015, false);
        return;
      }
      ...
      const healthFraction = "".concat(gameState.player.health, "/").concat(gameState.player.healthMax);
      const textContent = "    You bought ".concat(healAmount, " HP\nTotal health: ").concat(healthFraction);
    ...
    function handleShopAddCardPointerup(sE) {
      const {
        isAddCardEnabled
      } = IsShopButtonEnabled(sE.addCardCost);
      if (!isAddCardEnabled) {
        self.cameras.main.shake(50, .0015, false);
        return;
      }
    ...
    function handleShopRemoveCardPointerup(sE) {
      const {
        isRemoveCardEnabled
      } = IsShopButtonEnabled(sE.removeCardCost);
    ...
    function IsShopButtonEnabled(cost) {
      let levelComplete = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    ...
    function addPermanent(card) {
      ...
      if (gameState.fightStarted) {
        ...
        (_card$sprite = card.sprite) === null || _card$sprite === void 0 || _card$sprite.destroy();
      }
    ...
    const depleteFunctionRegistry = {
      depleteForeverTrue: card => _depleteForeverTrue(card),
      depleteRebelSpirit: card => _depleteRebelSpirit(card),
      depleteRebelHeart: card => _depleteRebelHeart(card),
      depleteBushido: card => _depleteBushido(card),
      depleteToxicAvenger: card => _depleteToxicAvenger(card),
      depleteKirisuteGomen: card => _depleteKirisuteGomen(card),
      depleteToxicFrets: card => _depleteToxicFrets(card),
      depleteAshenEncore: card => _depleteAshenEncore(card),
      depleteEdoEruption: card => _depleteEdoEruption(card),
      depleteSteelToe: card => _depleteSteelToe(card),
      depleteDeadTokugawas: card => _depleteDeadTokugawas(card),
      depleteGundanSeizai: card => _depleteGundanSeizai(card),
      depleteLustForLife: card => _depleteLustForLife(card),
      depletePunksNotDead: card => _depletePunksNotDead(card),
      depleteSoulSquatter: card => _depleteSoulSquatter(card),
      depleteBouncingSoles: card => _depleteBouncingSoles(card),
      depleteEnduringSpirit: card => destroyToken(card),
      depleteShogunsShell: card => _depleteShogunsShell(card),
      depleteZaibatsuU: card => _depleteZaibatsuU(card),
      // Non-depleted token cards
      depleteKamishimoUberAlles: function depleteKamishimoUberAlles(card, tokenSprite, tokenSlot) {
      ...
      depleteHollidayInKamakura: function depleteHollidayInKamakura(card, tokenSprite, tokenSlot) {
        if (tokenSlot) tokenSlot.available = true;
        if (tokenSprite) tokenSprite.destroy();
        if (card.permanentCardSprite) card.permanentCardSprite.destroy();
        gameState.permanents = gameState.permanents.filter(p => p.tokenSprite !== tokenSprite);
      ...
      depleteChemicalWarfare: function depleteChemicalWarfare(card, tokenSprite, tokenSlot) {
      ...
      depleteChintaiShunyu: function depleteChintaiShunyu(card, tokenSprite, tokenSlot) {
        if (tokenSlot) tokenSlot.available = true;
        if (tokenSprite) tokenSprite.destroy();
        if (card.permanentCardSprite) card.permanentCardSprite.destroy();
        gameState.permanents = gameState.permanents.filter(p => p.tokenSprite !== tokenSprite);
      ...
      depleteStageInvasion: function depleteStageInvasion(card, tokenSprite, tokenSlot) {
        if (tokenSlot) tokenSlot.available = true;
        if (tokenSprite) tokenSprite.destroy();
        if (card.permanentCardSprite) card.permanentCardSprite.destroy();
        gameState.permanents = gameState.permanents.filter(p => p.tokenSprite !== tokenSprite);
      ...
      depleteLaidoSoho: function depleteLaidoSoho(card, tokenSprite, tokenSlot) {
    ...
    function handleLaidoSohoPointerup(enemy, card, tokenSprite, tokenSlot) {
      ...
      addInfoTextBox("Dealt ".concat(dmg, " Physical Damage!"));
      if (endFightIfGameOver()) return;
    }
    ...
    function _depleteKirisuteGomen(card) {
    ...
    function _depleteSteelToe(card) {
      let depletionTriggeredByActivation = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      destroyToken(card);
    ...
    function removeSteelToe2FromGame() {
      ...
      const deckKeys = ['currentCards', 'deck', 'drawPile', 'discardPile', 'bonusCards', 'extraCards'];
    ...
    function _depleteRebelHeart(card) {
    ...
    function _depleteBushido(card) {
      gameState.bushido = false;
      destroyToken(card);
      gameState.player.strengthBase += 6;
      gameState.player.updateStats();
    }
    function _depleteRebelSpirit(card) {
    ...
    function _depleteForeverTrue(card) {
    ...
    function _depleteLustForLife(card) {
      destroyToken(card);
    ...
    function _depletePunksNotDead(card) {
      ...
      gameState.player.heal(Math.round(gameState.player.healthMax * 0.2));
      gameConfig.healSound.play({
        volume: 0.5
      });
    }
    function _depleteBouncingSoles(card) {
      if (gameState.bonusPermanentSlots.length) {
    ...
    function _depleteShogunsShell(card) {
    ...
    function _depleteToxicFrets(card) {
    ...
    ;
    function _depleteAshenEncore(card) {
    ...
    function activatePermanentFromHand(card) {
      ...
      if (typeof depleteFunctionRegistry[functionName] === 'function') {
        ...
        console.log("Unknown card key: ".concat(card.key));
      }
    }
    ...
    function displayTokenCard(card) {
      card.tokenSprite.on('pointerover', function () {
        ...
        addInfoTextBox("Available Permanent Slots: ".concat(numOfAvailableSlots, " / ").concat(numberOfSlots), 2000);
        gameConfig.cardsDealtSound.play({
          volume: 1.5,
          seek: 0.10
        });
    ...
    function activateHealButton() {
      ...
      gameState.lustForLifeButton.setPointerText(" Heal 7 HP\nCost: ".concat(gameState.lustForLifeCost, " gold"));
      const textContent = "Bought ".concat(healAmount, " HP");
      addInfoTextBox(textContent);
    }
    ...
    // Method for discarding the whole hand. Used for redrawing.
    ...
    function addInfoTextBox(textContent) {
      let delayTilDestruction = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1700;
    ...
    function updateAndDisplyPoisonCount(character, delayBeforeFadeOut) {
      ...
      const posionTextContent = "".concat(character.name, " takes ").concat(lostHP, " damage from Poison");
    ...
    function drawNewCards(numCards) {
      ...
      for (let i = 0; i < numCards; i++) {
        self.time.delayedCall(i * 75, () => {
          ...
          if (card.usedOneShot) card.sprite.setTint(0x808080);
          console.log("card added: ".concat(card.key));
          card.activate();
          card.isBeingPlayed = false;
          gameState.currentCards.push(card);
    ...
    async function dealAshenDmgOnDrawCard() {
      ...
      const textContent = "Deals ".concat(ashenDamage, " fire damage to all enemies");
      addInfoTextBox(textContent);
      gameState.enemies.forEach(enemy => {
    ...
    function fadeOutGameObject(gameObject) {
      let duration = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 200;
      //NB! Keep it here. Moving it to base => bugs!
    ...
    function addAdminTools() {
      ...
      gameState.killEnemyButton.on('pointerup', () => {
        ...
        if (gameState.enemies.some(enemy => enemy.health > 0 && gameState.playersTurn)) {
          ...
          enemy.removeIfDead(handleTroopsOfTakamori);
          endFightIfGameOver();
          console.log("".concat(enemy.name, " killed"));
        }
      });
    }
