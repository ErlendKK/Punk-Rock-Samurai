...
class BaseScene extends Phaser.Scene {
  ...
  // Initiaiate level
  baseCreate(backgroundImage, sceneKey) {
    let scale = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1.13;
  ...
  resetPlayer(scale) {
    let x = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 360;
    let y = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 350;
    ...
    player.numCards = 5, player.numCardsBase = 5;
  ...
  defineCardSlots() {
    ...
    gameState.slots = [{
      available: true,
      x: x + 0 * spacing,
      y: y + 6 * hightAdjustment,
      index: 0,
      angle: -3 * cardAngle
    }, {
      available: true,
      x: x + 1 * spacing,
      y: y + 3 * hightAdjustment,
      index: 1,
      angle: -2 * cardAngle
    }, {
      available: true,
      x: x + 2 * spacing,
      y: y + 1 * hightAdjustment,
      index: 2,
      angle: -1 * cardAngle
    }, {
      available: true,
      x: x + 3 * spacing,
      y: y + 0 * hightAdjustment,
      index: 3,
      angle: 0 * cardAngle
    }, {
      available: true,
  ...
  definePermanentSlots() {
    ...
    gameState.permanentSlots = [{
      available: true,
      x: permanentX + 0 * permanentSpacing,
      y: permanentY,
      index: 0
    }, {
      available: true,
      x: permanentX + 1 * permanentSpacing,
      y: permanentY,
      index: 1
    }, {
      available: true,
      x: permanentX + 2 * permanentSpacing,
      y: permanentY,
      index: 2
    }, {
      available: true,
      x: permanentX + 3 * permanentSpacing,
      y: permanentY,
      index: 3
    }, {
      available: true,
      x: permanentX + 4 * permanentSpacing,
      y: permanentY,
      index: 4
    }];
  }
  ...
  addGoldCounter() {
    ...
    gameState.goldCounter = this.add.text(1570, 76, gameState.player.gold, {
      fontSize: '70px',
      fill: '#000000'
    }).setDepth(221).setOrigin(.5);
    let currentInput = "";
  ...
  describeCharacter(character) {
    character.sprite.on('pointerover', () => {
      ...
      const fullText = "".concat(character.name, "\n\n").concat(character.armor, " armor: Take ").concat(Math.round(Math.abs(character.armor / 20 * 100)), " %\n") + "".concat(armorText, " physical damage\n\nMax armor: Take 75 %\nless physical damage \n\n").concat(character.strength, " Strength: Deal ") + "".concat(Math.abs(character.strength) * 10, " %\n").concat(strengthText, " physical damage\n\nMax strength: Deal 150 %\nmore physical ") + "damage\n\nPoison: ".concat(character.poison, "\nPoison change per turn: ").concat(character.poison > 0 ? -1 : 0);
      const textCoordinates = {
        x: character.sprite.x - character.width / 2 - 100,
        y: 490,
        z: 30
      };
      const textConfig = {
  ...
  addStanceBar(character, textColor) {
    ...
    let stanceTextConfig = {
      fontSize: '40px',
      fill: textColor,
      fontWeight: 'bold'
    };
    gameState.stanceText = this.add.text(825, gameState.stanceBarMargin - 45, "Stance: ".concat(gameState.player.stance), stanceTextConfig).setOrigin(0.5).setInteractive();
    gameState.stanceText.on('pointerover', () => {
      console.log("Stance: ".concat(gameState.player.stance));
  ...
  clearBoard() {
    ...
    const gameObjectsToDestroy = [gameState.player.manaBarBackground, gameState.player.manaBarFrame, gameState.player.manaBar, gameState.player.manaBarText, gameState.drawPileImage, gameState.discardPileImage, gameState.drawPileText, gameState.discardPileText, gameState.stanceBar, gameState.stanceText, gameState.stanceBarBackground, gameState.endOfTurnButton, gameState.redrawButton, gameState.healButton, gameState.actionText];
  ...
  updateTextAndBackground(textObj, backgroundObj, newText) {
    let cornerRadius = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 7;
    let depth = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 20;
    let alpha = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 0.60;
    ...
    gameState.permanents.forEach(permanent => {
      if (gameState.permanentSlots.some(slot => slot.available)) {
        ...
        if (!gameConfig.tokenCardNames.some(item => item.key === card.key)) {
          console.log("Recreated permanent: ".concat(card.key, "\nfor slot ").concat(card.slot.index));
          addPermanent(card);
        }
      } else {
  ...
  // In case of failue; try again 5 times at increasing time intervals
  loadAssetWithRetry(asset, type, attempt, maxRetries) {
    ...
    this.load.once("filecomplete-".concat(type, "-").concat(asset.key), () => {
      console.log("".concat(asset.key, " loaded successfully."));
    });
    this.load.once('loaderror', file => {
      if (file.key === asset.key) {
        console.warn("Loading error for asset: ".concat(asset.key, ". Attemt: ").concat(attempt, " Retrying..."));
