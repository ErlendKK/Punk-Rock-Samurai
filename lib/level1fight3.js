...
class Level1Fight3 extends BaseScene {
  constructor() {
    super('Level1Fight3');
  }
  preload() {
    const assets = [{
      key: 'bakgrunnCity3',
      path: '../assets/images/backgrounds/bakgrunnCity3.jpg',
      loaded: false
    }, {
      key: 'police1',
      path: '../assets/images/sprites/police1.png',
      loaded: false
    }, {
      key: 'police2',
      path: '../assets/images/sprites/police2.png',
      loaded: false
    }];
  ...
  create() {
    ...
    gameState.enemy1 = new Enemy('Sgt. Pig', 45, 0, 0, '');
    gameState.enemy1.sprite = this.add.image(1015, 530, 'police1').setScale(0.372).setFlipX(true);
    gameState.enemy1.actions = [
    // Predetermined turns
    new Action({
      enemy: 1,
      turn: 1
    }).setKey("Intends to\nRequest backup"), new Action({
      enemy: 1,
      pattern: 'even',
      damage: 12
    }).setKey(""), new Action({
    ...
    const enemy2health = self.adjustForDifficulty(55, 60, 65);
    gameState.enemy2 = new Enemy('Lt. Pig', enemy2health, 0, 0, '');
    gameState.enemy2.sprite = this.add.image(1255, 530, 'police2').setScale(0.570).setFlipX(true);
    ...
    async function initiatePlayerTurn() {
      ...
      gameState.turn++;
      if (!policeEvents()) return; //NB! Particular for level1fight3
    ...
    function displayCountdownBox() {
      const turnsTextConfig = {
        fontSize: '80px',
        fontWeight: 'bold',
        fill: '#000000'
      };
      gameState.turnsTextBox = self.add.rectangle(825, 225, 83, 83, 0xffffff).setAlpha(0.30).setOrigin(0.5);
      gameState.turnsText = self.add.text(825, 230, "".concat(turnsTillPoliceArrive), turnsTextConfig).setOrigin(0.5);
    }
    ...
    */
    ...
    async function initiateEnemyTurnOneEvent() {
      ...
      const enemTurnOneText = new TextBox(self, textContent, textCoordinates, textConfig, 0.6);
      await self.delay(secondDelayTime);
      enemTurnOneText.destroy();
      const policeTextContent = 'Police snipers\n will hit you\n  in 5 turns!';
      const policeText = new TextBox(self, policeTextContent, textCoordinates, textConfig, 0.6);
