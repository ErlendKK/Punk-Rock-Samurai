class Level2fight3 extends BaseScene {
    ...
    preload() {
        this.load.image('bakgrunnForest3', 'assets/images/bakgrunnForest3.jpg');
        this.load.image('tree3', 'assets/images/sprites/tree3.png');
        };

    create() {   
        let self = this;      
        this.baseCreate('bakgrunnForest3'); 

        // Set up characters
        this.resetPlayer(gameState.player); //BaseScene

        gameState.enemy1 = {
            name: 'Lord of the Trees\nLose Armor when attacked\nTakes 2x damage from Fire',
            sprite: this.add.sprite(750, 325, 'tree3').setScale(0.36).setFlipX(false).setInteractive(), //690 / 350 / .33
            alive: true,
            actions: [],         
            health: 110,
            healthMax: 110,
        ...
        gameState.enemy2 = {
            name: 'Tree (2x damage from Fire)',
            sprite: this.add.sprite(910, 332, 'goblin1').setScale(0.01).setFlipX(true).setInteractive(), //910 / 310 / .25
            alive: true,
            actions: [],        
            health: 0, // Set = 0 to reduce the number of enemies to 1
            healthMax: 70,
            strength: 0,
            strengthBase: 0,
            strengthMax: 15, 
    ...
    function updateEnemyActions() {
        ...
        if (gameState.turn === 1) {
            gameState.enemy1.actions = [
                {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 4, armor: 0, text: 'Gains 4 Strength', probability: 1},
            ]
            
        } else if (gameState.turn === 2) {
            gameState.enemy1.actions = [
                {key: `Intends to\nRest`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 3, armor: 0, text: 'Gains 3 Strength', probability: 1},
            ]
            
        } else if (gameState.turn === 3) {
            gameState.enemy1.actions = [
                {key: () => `Intends to\nDeal ${Math.round(20 * (1 + 0.10 * gameState.enemy2.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 15, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 15 damage', probability: 1},
            ]


        } else if (gameState.turn === 4) {
            gameState.enemy1.actions = [
                {key: `Intends to\nRest`, damage: 0, fire: 0, poison: 0, heal: 4, poisonRemove: 0, strength: 0, armor: 0, text: 'Rests', probability: 1},
            ]
            ...
            gameState.enemy1.actions = [ 
                {key: `Intends to\nDeal 6 fire damage`, damage: 0, fire: 6, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: `Deals 6 fire damage`, probability: 0.10 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5},
                {key: () => `Intends to\nDeal ${Math.round(12 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 12, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 12 damage', probability: 0.24 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5},
        ...
        gameState.enemy2.actions = [ 
            ...
            {key: () => `Intends to\nDeal ${Math.round(10 * (1 + 0.10 * gameState.enemy2.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 10, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 10 damage', probability: 0.235 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 5},
            {key: () => `Intends to\nDeal ${Math.round(15 * (1 + 0.10 * gameState.enemy2.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 15, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 15 damage', probability: 0.17 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 5},
    ...
    function selectEnemyAction(enemy) {
        ...
        function chooseReward() {
            ...
            gameState.rewardAddCardsButton = self.add.image(550,350, 'rectangularButton').setScale(1.2).setInteractive().setOrigin(0.5).setDepth(102); // Dept > 100 is to ensure that it stays on top of card sprites that fail to clear
            ...
            gameState.rewardRemoveCardButton = self.add.image(550, 500, 'rectangularButton').setScale(1.2).setInteractive().setOrigin(0.5).setDepth(102);;
            gameState.rewardRemoveCardText = self.add.text(550, 500, 'Remove 1 card\nfrom your deck', { fontSize: '25px', fill: '#000000' }).setOrigin(0.5).setDepth(103);;
        ...
        function rewardRemoveCard() {
            ...
            deck.forEach( (deckCard, index) => {
                ...
                deckCard.sprite.on('pointerover', function() {
                    gameState.cardsDealtSound.play({ volume: 0.6 });
                    cardTweens(deckCard.sprite, 0.40, 200);
                    deckCard.sprite.setDepth(200);
                }, this)
        
                deckCard.sprite.on('pointerout', function() {
                    cardTweens(deckCard.sprite, 0.27, 400);
