...
class Level4Fight1 extends BaseScene {self
    ...
    create() {
        const self = this;      
        this.baseCreate('bakgrunnFortress1'); //BaseScene method  
    ...
    function startPlayerTurn() { //NB! LEVEL-SPECIFIC FUNCTION: don't copy elsewhere!
        ...
        // NB! Stance must be reset/kept after mana
        ...
        self.time.delayedCall(2200, () => { //timer: 2200
            ...
            if (gameState.turn === 1) {
                ...
                let treesInfoText  = self.add.text(550, 300, `Monsters steal Armor or\nStrength when they attack`, textConfig).setOrigin(0.5).setDepth(201);
    ...
    function updateEnemyActions() {
        ...
        if (gameState.turn === 1) {
            gameState.enemy1.actions = [
                {key: `Intends to\nSteal Armor`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 3, reduceTargetStrength: 0, reduceTargetArmor: 3, text: 'Steals 3 armor', probability: 1},
            ] 
        } else if (gameState.turn === 2) {
            gameState.enemy1.actions = [
                {key: () => `Intends to\nDeal ${Math.round(12 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 12, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 1, reduceTargetStrength: 0, reduceTargetArmor: 1, text: 'Deals 12 damage', probability: 1},
            ]
            
        } else if (gameState.turn === 3) {
            gameState.enemy1.actions = [
                {key: () => `Intends to\nDeal ${Math.round(15 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 15, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, reduceTargetStrength: 0, reduceTargetArmor: 0, text: 'Deals 15 damage', probability: 1},
            ]
        ...
        if (gameState.turn === 1) {
            gameState.enemy2.actions = [
                {key: `Intends to\nSteal Strength`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 3, armor: 0, reduceTargetStrength: 3, reduceTargetArmor: 0, text: 'Steals 3 Strength', probability: 1},
            ] 
    ...
    function selectEnemyAction(enemy) {
        ...
        function startNextLevel() {
            ...
            self.scene.start('Level4Fight2'); //sett lik neste level
            });
        }
