class Level2fight2 extends BaseScene {
    ...
    preload() {
        this.load.image('bakgrunnForest2', 'assets/images/bakgrunnForest2.jpg');
        this.load.image('goblin1', 'assets/images/sprites/goblin1.png');
        this.load.image('goblin2', 'assets/images/sprites/goblin2.png');
        };
    ...
    create() {   
        ...
        gameState.enemy1 = {
            name: 'Junior Goblin\nMaster of poison',
            sprite: this.add.sprite(690, 365, 'goblin1').setScale(0.21).setFlipX(true).setInteractive(), //690 / 350 / .33
            alive: true,
            actions: [],         
            health: 50,
            healthMax: 50,
        ...
        gameState.enemy2 = {
            name: 'Senior Goblin\nMaster of poison',
            sprite: this.add.sprite(870, 360, 'goblin2').setScale(0.25).setFlipX(false).setInteractive(), //910 / 310 / .25
            alive: true,
            actions: [],        
            health: 60, // Set = 0 to reduce the number of enemies to 1
            healthMax: 60,
            strength: 0,
            strengthBase: 0,
            strengthMax: 15, 
        ...
        gameState.stanceText.on('pointerover', function() {
                ...
                gameState.stanceDescriptionBackground = self.add.sprite(550, gameState.stanceBarBackground.y + 50, 'listbox2').setScale(3.1, 7.4).setInteractive().setDepth(20).setOrigin(0.5, 0).setAlpha(0.85);
                gameState.stanceDescriptionText = self.add.text(550, gameState.stanceBarBackground.y + 97, stanceText, { fontSize: '14px', fill: '#000000' }).setDepth(25).setOrigin(0.5, 0);
        })
        ...
        function describeCharacter(character) {
            if (!gameState.targetingCursor.visible && character.alive === true) {
                character.sprite.on('pointerover', function() {
                    ...
                    character.descriptionBackground.fillRect(0, 0, 220, 250);
        ...
        function startFight() {
            ...
            self.time.delayedCall(1500, () => { //timer: 1500
    ...
    // ---------------------------------- PLAYERS TURN -------------------------------------    
        ...
        function startPlayerTurn() {
            ...
            self.time.delayedCall(1800, () => { //timer: 2000
                ...
                gameState.enemies.forEach( enemy => {
                    ...
                    if (!enemy.chosenAction) enemy.chosenAction = 'intention is secret'
        ...
        function playCard(card, target) {
            ...
            let damageTotal = Math.round(Math.max(0, firePlayed + damagePlayed * (1 + 0.10 * gameState.player.strength) * (1 - target.armor / 20)));
    ...
    function performEnemyAction(enemy) {
        ...
        self.time.delayedCall(delaytime, () => {
            ...
            if (chosenAction.damage > 0 || chosenAction.fire > 0) {
                ...
                gameState.actionText = self.add.text(550, 300, `Deals ${enemy.damageTotal} damage\nAnd poisons you`, { fontSize: '32px', fill: '#ff0000' }).setOrigin(0.5);
    ...
    function updateEnemyActions() {
        gameState.enemy1.actions = [ 
            {key: `Intends to\nDeal 5 fire damage`, damage: 0, fire: 5, poison: 1, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: `Deals 5 fire damage`, probability: 0.00 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 6},
            {key: () => `Intends to\nDeal ${Math.round(8 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 8, fire: 0, poison: 1, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 8 damage\nAnd 1 Poison', probability: 0.22 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 6},
            {key: () => `Intends to\nDeal ${Math.round(12 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 12, fire: 0, poison: 1, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 12 damage\nAnd 1 Poison', probability: 0.10 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 6},
            ...
            {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 5, text: 'Gains 5 armor', probability: 0.15 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 6},
            {key: `Intends to\nPoison you`, damage: 0, fire: 0, poison: 2, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 2 poison', probability: 0.19}
        ]
    
        gameState.enemy2.actions = [ 
            {key: `Intends to\nDeal 7 fire damage`, damage: 0, fire: 7, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 7 fire damage', probability: 0.0 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 6},
            {key: () => `Intends to\nDeal ${Math.round(10 * (1 + 0.10 * gameState.enemy2.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 10, fire: 0, poison: 2, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 10 damage\nAnd 2 Poison', probability: 0.16 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 6},
            {key: () => `Intends to\nDeal ${Math.round(15 * (1 + 0.10 * gameState.enemy2.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 15, fire: 0, poison: 2, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 15 damage\nAnd 2 Poison', probability: 0.11 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 6},
            ...
            {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 3, armor: 0, text: 'Gains 3 strenght', probability: 0.12 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 6},
            {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 4, text: 'Gains 4 armor', probability: 0.17 + ((gameState.enemy2.health >= gameState.enemy2.healthMax) ? 0.22 : 0) / 6},
            {key: `Intends to\nPoison you`, damage: 0, fire: 0, poison: 3, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 3 poison', probability: 0.22}
        ]
    ...
    // ---------------------------------- UTILITIES-------------------------------------      
        ...
        gameState.muteText = this.add.text(875, 30, 'Mute music', { fontSize: '25px', fill: '#a9a9a9' });    
