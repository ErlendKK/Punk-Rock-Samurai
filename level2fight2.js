...
class Level2Fight2 extends BaseScene {self
    ...
    create() {
        const self = this;      
        this.baseCreate('bakgrunnForest2'); //BaseScene method  
        ...
        gameState.enemy1.sprite = this.add.sprite(670, 370, 'goblin1').setScale(0.19).setFlipX(true).setInteractive(), //690 / 350 / .33
        ...
        gameState.enemy2.sprite = this.add.sprite(800, 370, 'goblin3').setScale(0.17).setFlipX(false).setInteractive(); //710 / 350 / .33
        ...
        gameState.enemy3.sprite = this.add.sprite(930, 370, 'goblin2').setScale(0.23).setFlipX(false).setInteractive(), //910 / 310 / .25
    ...
    function selectEnemyAction(enemy) {
        ...
        function initiateDefeat() {
            ...
            const gameOverTextConfig = { fontSize: '100px', fill: '#ff0000', fontFamily: 'Rock Kapak' }

            gameState.gameOverText = self.add.text(550, 300, ' Defeat!', gameOverTextConfig).setOrigin(0.5);
        ...
        function chooseReward() {
            ...
            gameState.rewardAddCardsButton = self.add.image(550, 400, 'rectangularButton')
    ...
    // ---------------------------------- PERMANENTS-------------------------------------      
            ...
            function depleteAshenEncore(card) {
                console.log(`DEBUG: depleteAshenEncore started`)
                gameState.ashenEncore = false;
                if (card.tokenSlot) card.tokenSlot.available = true;
                if (card.sprite) card.sprite.destroy();
                if (card.tokenSprite) card.tokenSprite.destroy();
    ...
    // ---------------------------------- UTILITIES-------------------------------------      
        ...
        function normalizeCard(card, target, isLastEnemy = false) {
            ...
            return {
                ...
                firePlayed: kabutuEdoCondition ? 2 * gameState.player.stancePoints : scorchedSoulCondition ? 12 : getValueOrInvoke(card.fire),
