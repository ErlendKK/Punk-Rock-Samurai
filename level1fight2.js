...
class Level1Fight2 extends BaseScene {self
    constructor() {
        super('Level1Fight2');
    }

    preload() {
        this.load.image('bakgrunnCity2', 'assets/images/bakgrunnCity2.jpg');
        this.load.image('rat1', 'assets/images/sprites/rat1.png');
        this.load.image('rat2', 'assets/images/sprites/rat2.png');
        this.load.image('ratCard', 'assets/images/cards/ratCard.jpg'); 
    }; 

    create() {
        const self = this;
        this.saveGameState(self.scene.key);
        this.baseCreate('bakgrunnCity2');
        this.resetPlayer(gameState.player, 0.48, 360, 285); //l1f1:0.28 -- l1f2: 0.24, 360, 300 (liten:0.48, 360, 280)
        ...
        async function exchangeTaunts() {
            ...
            if (!gameState.ratTaunts) {
                gameState.ratTaunts = gameState.extraTaunts 
            }
    ...
    // ---------------------------------- PLAYERS TURN -------------------------------------    
        ...
        function startPlayerTurn() {
            ...
            gameState.player.poisonText = self.add.text(540, 360, '', { fontSize: '30px', fill: '#ff0000' }).setOrigin(0.5);
        ...
        function initiateTurnOne(numCards) {
            self.time.delayedCall(300, () => {
                ...
                const ratsTextContent = "Heads up: Rats\nare infectious!"
                const ratsText  = self.add.text(550, 280, "", textConfig).setOrigin(0.5).setDepth(201);
                const ratsTextBackground = self.add.graphics();
                self.updateTextAndBackground(ratsText, ratsTextBackground, ratsTextContent)
        ...
        function playCard(card, target, isLastEnemy = false) {
            ...
            if (target != gameState.player) {
                ...
                if (reduceTargetArmorPlayed) console.log(`reduceTargetArmorPlayed: ${reduceTargetArmorPlayed}`);
        ...
        function addTextAndTweens(damageTotal, poisonPlayed, strengthPlayed, armorPlayed, poisonRemovePlayed) {
            ...
            gameState.actionText = self.add.text(550, 280, "", actionTextConfig).setOrigin(0.5).setDepth(21);
    ...
    // ---------------------------------- ENEMY'S TURN -------------------------------------      
        ...
        function performEnemyAction(enemy) {
            ...
            self.time.delayedCall(delaytime, () => {
                ...
                if (chosenAction.infect) {
                    insertInfestationCard(enemy);
                } else {
                    gameState.infection = false;
                }
        ...
        function concludeEnemyAction(enemy, chosenAction) {
            ...
            const delaytime = gameState.infection ? 2200 : 1300
        ...
        function insertInfestationCard(enemy) {
            return new Promise((resolve) => {            
                ...
                const ratCardOne = self.add.image(x, y, 'ratCard').setScale(scale).setOrigin(0.5).setDepth(300);
                    ...
                    ratCards.forEach((ratCard) => {
                        self.tweens.add({
                            ...
                            onComplete: function () {
                                ...
                                gameState.drawPileText.setText(gameState.drawPile.length);
                                completedTweens++;
                                if (completedTweens === ratCards.length) {
                                    resolve();
                                }
                            },
                        });
        ...
        function updateEnemyActions() {
            ...
            if (gameState.turn === 1) {
                gameState.enemy1.actions = [
                    {key: `Intends to\nPoison you`, damage: 0, fire: 0, poison: 5, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 5 poison', probability: 1},
                ] 

            } else if (gameState.turn === 2) {
                gameState.enemy1.actions = [
                    {key: `Intends to\nApply a buff`, damage: 0, fire: 0, poison: 0, heal: 10, poisonRemove: 0, strength: 0, armor: 4, text: 'Heals 10 HP\nGains 4 Armor', probability: 0.10 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5}
                ]
                ...
                gameState.enemy1.actions = [ 
                    {key: `Intends to\nDeal 5 poison`, damage: 0, fire: 0, poison: 5, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: `Deals 5 poison`, probability: 0.10 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5},
                    ...
                    {key: () => `Intends to\nDeal ${Math.round(15 * (1 + 0.10 * gameState.enemy1.strength) * (1 - gameState.player.armor / 20))} damage`, damage: 15, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 15 damage', probability: 0.18 + ((gameState.enemy1.health >= gameState.enemy1.healthMax) ? 0.22 : 0) / 5},
                    ...
                    {key: `Intends to\nPoison you`, damage: 0, fire: 0, poison: 4, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 5 poison', probability: 0.00}
                ]
            }

            if (gameState.turn === 1) {
                gameState.enemy2.actions = [
                    {key: `Intends to\nInfect you`, damage: 0, fire: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Infects you', infect: true, probability: 1},
                ] 
